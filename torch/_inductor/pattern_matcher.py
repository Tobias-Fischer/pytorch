...
class MatchContext:
    ...
    def __init__(self, outputs: List["PatternExpr"], pattern_to_node: Optional[Dict["PatternExpr", Node]]=None):
    ...
    def filter_multi_user_patterns(self):
        return {pattern: node for pattern, node in self.pattern_to_node.items() if pattern.has_multiple_users()}
...
class CallFunction(PatternExpr):
    ...
    def _match(self, node: torch.fx.Node, ctx: MatchContext):
        ...
        if self not in ctx.outputs and self.users is not MULTIPLE and len(node.users) != self.users:
